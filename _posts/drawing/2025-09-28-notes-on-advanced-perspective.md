...
椭圆模板的角度的定义是椭圆焦点到短轴顶点的连线与长轴之间的夹角。注意这里的角度并不是其在透视画面中的视觉角度（angle of view）。虽然它也的值也是通常范围从 0°（一条平直的线）到 90°（一个完整的圆）。以下图蓝色的45°椭圆为例。首先，将一段长度与长轴相等的线段的一端与短轴顶点重合、另一端与长轴相交，即可得到该椭圆的焦点（根据椭圆焦点的公式：长轴^2 = 短轴^2 + 焦点到圆心距离^2）。此时椭圆模板的角度指的就是这里椭圆焦点到短轴顶点与长轴之间的夹角。
