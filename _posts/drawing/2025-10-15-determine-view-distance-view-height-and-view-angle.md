...
### 根据物体实际大小和视距确定其在画面中的大小和位置

例如，已知观察者坐在长度为1米的方形桌子前，以45度角俯视桌子，此时画面中可以看到整个桌子，观察者的视线（direction of view）正好穿过桌子表面的1/2处，要求画出当画面距离观察者视距为80厘米时，桌子在60度视锥中的画面大小和位置。

1. 根据画面要求推导出画面的侧视图（包括观察者的位置和视线方向）；
2. 依次在侧视图中画出90度视锥（即两条被视线平分的夹角为90度的线）和60度视锥；
3. 根据侧视图中物体的画面大小和实际大小得出画面的缩放比例（比如，画面中桌子表面的长度是10厘米，而已知桌子的长度是1米，可知侧视图的缩放比例是1/10）；
...
至此我们已经确定了物体的上下轮廓在画面中出现的位置，接下来我们还需要确定物体不同部位在画面中水平方向的宽度（比如桌子上下边缘在画面中的宽度）。

我们可以通过将物体俯视图
根据物体上下轮廓在画面中出现的位置绘制画面的草图（）；
1. 根据视距和画一幅画面的俯视图


在画面上的投影尺寸和物体距离观察者的距离以及视距这3者之间的比例关系可以算出物体，根据该物体的实际大小，可以计算出

### 根据物体实际大小和视距确定其在画面中的大小和位置

例如，已知观察者坐在长度为1米的方形桌子前，以45度角俯视桌子，此时画面中可以看到整个桌子，观察者的视线正好穿过桌子表面的1/2处，求桌子在60度视锥中的画面大小和位置。

1. 画一幅画面的草图，此时不用考虑透视问题（比如，视距，视高，视锥角度等）；
2. 根据画面要求推导出画面的侧视图（包括观察者的位置和视线方向与地面之间的角度）；
3. 依次在侧视图中画出90度视锥（即两条被视线平分的夹角为90度的线）和60度视锥；
4. 沿着视线方向前后移动连同60度视锥一起移动观察者的位置，当侧视图中出现在60度视锥中的物体与画面中一致时，即可确定画面平面的位置，进而确定视距的测量长度（比如假如出现在画面下边缘的正好是桌子表面的底边，那么可以在侧视图中找到一个观察点，其60度视锥的下边缘正好穿过桌子表面的底边，此时沿着桌子底边画一条垂直于视线方向的线条即为画面平面）；
5. 根据侧视图中画面所在平面在60度视锥之间的测量长度可推算出画面的视距的测量长度（即60度视锥半径的1.72倍，即1 / 0.58）；
3. 找到画面中处于画面平面中（即与画面平面距离为0）的某个已知大小的物体（比如假设根据推算得知桌子表面的1/2处正好处于画面平面，测量得知其在画面中的宽度为10厘米）；
4. 根据该物体的实际大小，可以计算出
